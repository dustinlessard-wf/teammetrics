...
class PrController extends HTTPController {
  ...
  @httpGet
  Future<Response> getPrs(@HTTPQuery("repos") String repos, {@HTTPQuery("limit") int limit : 10,
    ...
    for (var repoName in repoList) {
      ...
      final rawPullRequests = github.pullRequests
          .list(repoSlug);
    ...
    String prHtml = '';
    String headHtml = '<head><link rel="stylesheet" href="https://cdn.wdesk.com/home/3.0.13/packages/web_skin/dist/css/web-skin.min.css"></head>';
    ...
    prs.forEach((git.PullRequest pr){
      ...
      prHtml += '<div class="panel ${panelclass}"><div class="panel-heading">${pr.title}$age</div><div class="panel-body"><label>created at:</label> ${pr.createdAt}<br/><label>url:</label> <a target="_blank" href="${pr.htmlUrl}">${pr.htmlUrl}</a></div></div>';
